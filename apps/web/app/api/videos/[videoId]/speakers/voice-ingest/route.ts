...
import { IngestVoiceResponseSchema } from "@yt/contracts";
...
export async function POST(req: Request, ctx: { params: Promise<{ videoId: string }> }) {
  ...
  try {
    ...
    try {
      ...
      if (!video) {
        metrics.httpRequestsTotal.inc({ route: "/api/videos/:id/speakers/voice-ingest", method: "POST", status: "404" });
      ...
      await queue.add("ingest_voice", jobData, {
        jobId: job.id,
        removeOnComplete: true,
        removeOnFail: false,
      });

      metrics.httpRequestsTotal.inc({ route: "/api/videos/:id/speakers/voice-ingest", method: "POST", status: "200" });
      return NextResponse.json(IngestVoiceResponseSchema.parse({ job }), {
    ...
    metrics.httpRequestsTotal.inc({ route: "/api/videos/:id/speakers/voice-ingest", method: "POST", status: String(apiErr.status) });
