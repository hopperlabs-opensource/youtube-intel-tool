...
export async function GET(req: Request) {
  ...
  try {
    ...
    metrics.httpRequestsTotal.inc({ route: "/api/karaoke/playlists", method: "GET", status: String(apiErr.status) });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const apiErr = classifyApiError(err);
    metrics.httpRequestsTotal.inc({ route: "/api/karaoke/playlists", method: "POST", status: String(apiErr.status) });
    return jsonError(apiErr.code, apiErr.message, { status: apiErr.status, details: apiErr.details });
  }
}
